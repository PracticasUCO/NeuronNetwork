...
public class MainWindow extends JFrame {
	...
	private void initializeComponents() {
		...
		inertiaFactorPanel.setLayout(new BoxLayout(inertiaFactorPanel,
				BoxLayout.X_AXIS));
		...
		inertiaFactor
				.setToolTipText("Indica la fuerza que tendrá sobre el aprendizaje de la neurona los cambios producidos en iteraciones anteriores");
		...
		minimumImprovement
				.setToolTipText("Indica la mejora mínima entre cada retropropagación que debe mantenerse para continuar entrenando.");
		...
		hiddenLayersPanel.setLayout(new BoxLayout(hiddenLayersPanel,
				BoxLayout.X_AXIS));
		hiddenLayersPanel.setBorder(new TitledBorder("Capas ocultas"));
		hiddenLayers = new JSpinner(new SpinnerNumberModel(1, 1,
				Integer.MAX_VALUE, 1));
		hiddenLayers
				.setToolTipText("Indica el número de capas ocultas que tendrá la red neuronal");
		hiddenLayersPanel.add(hiddenLayers);
		spinnerOptionsPanel.add(hiddenLayersPanel);

		// Hidden neurons
		hiddenNeuronsPanel = new JPanel();
		hiddenNeuronsPanel.setLayout(new BoxLayout(hiddenNeuronsPanel,
				BoxLayout.X_AXIS));
		hiddenNeuronsPanel.setBorder(new TitledBorder("Neuronas ocultas"));
		hiddenNeurons = new JSpinner(new SpinnerNumberModel(1, 1,
				Integer.MAX_VALUE, 1));
		hiddenNeurons
				.setToolTipText("Indica el número de neuronas por capa oculta en la red neuronal");
		...
		maxiter = new JSpinner(new SpinnerNumberModel(1000, 300,
	...
	public double getLearningFactor() {
		return ((double) learningFactor.getValue() / (double) learningFactor
				.getMaximum());
	}

	public double getInertiaFactor() {
		return ((double) inertiaFactor.getValue() / (double) inertiaFactor
				.getMaximum());
	}
	...
	private void onLoadTrainButtonClicked() {
		...
		if (returnVal == JFileChooser.APPROVE_OPTION) {
			MainController.setTrainData(fileChooser.getSelectedFile()
					.getAbsolutePath());
		}
	}
	...
	private void onLoadTestButtonClicked() {
		...
		if (returnVal == JFileChooser.APPROVE_OPTION) {
			MainController.setTestData(fileChooser.getSelectedFile()
					.getAbsolutePath());
		}
	}
